    ...
    if (data.SystemState == 64 && info.length >0){
        ...
        $.each(info,function (key,val) {
            ...
            if (data.LockState == 1 && val.State >= 0){
                var $li = $('<li><a><dl class="orderList"><dd>用户名:&nbsp;' + val.UserName + '</dd><dd>返点:&nbsp;' + val.Rebate +'</dd><dd>状态:&nbsp;'+ getFenHongState(val.State)[0] +'</dd><dd><button style="display:none;" onclick="contract_ModifyCtt(this)" class="modifyCttBtn">'+ getFenHongState(val.State)[1] +'</button><button class="modifySettleBtn" onclick="contract_Settle(this)">分红结算</button></dd></dl></a></li>').data('fenHongCttInfo',modifyCttInfo);
            }else{
                var $li = $('<li><a><dl class="orderList"><dd>用户名:&nbsp;' + val.UserName + '</dd><dd>返点:&nbsp;' + val.Rebate +'</dd><dd>状态:&nbsp;'+ getFenHongState(val.State)[0] +'</dd><dd><button style="display:none;" onclick="contract_ModifyCtt(this)" class="modifyCttBtn">'+ getFenHongState(val.State)[1] +'</button></dd></dl></a></li>').data('fenHongCttInfo',modifyCttInfo);
            }
