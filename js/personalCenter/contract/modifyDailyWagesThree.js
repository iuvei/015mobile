...
function modifyDailyWagesThree_callBack(data) {
    ...
    if (data.SystemState == 64){
        ...
        if (modifyDayWageInfo.State == 2) {  //添加日工资契约
            ...
            for (var i = 0; i < initDailyWages.length;i++){
                if(parseFloat(initDailyWages[i].DayWagesProportion) <= parseFloat(my_daily_wages) && bigNumberUtil.multiply(initDailyWages[i].DayWagesProportion,100) >= data.MaxdDayWagesProportion){
